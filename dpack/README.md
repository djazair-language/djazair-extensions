...
- **`assets`** *(Array)*: An array of strings representing additional files or folders to include in the bundle. Paths are resolved relative to the `scriptPath`'s directory.
- **`quiet`** *(Boolean)*: If `True`, disables the build logs in the console.

### Example: Bundling a Game with Assets

If you have a game that relies on external files (e.g., sound effects and an `images/` folder), you must include them in the `assets` array so `dpack` knows to pack them:

```dz
use dpack

# Packs 'snake.dz' into 'snake_game.exe'
# Includes 'death.wav', 'eat.wav', and the entire 'assets/' directory
dpack.pack("snake.dz", "snake_game.exe", [], ["death.wav", "eat.wav", "assets/"])
```
